...
//Solution copied from https://github.com/gereons/
...
struct Day07: AdventDay {
    ...
    private var entites: [Hand] {
        //32T3k 765 ||  DRAW BET
        data.components(separatedBy: .newlines).map { Hand($0)}
    }
    
    func part1() throws -> Int {
        sumBids(entites.sorted(by: Hand.compareRank))
    }
    
    func part2() throws 
    -> Int {
        sumBids(entites.sorted(by: Hand.compareJokerRank))
    }
    
    private func sumBids(_ hands: [Hand]) -> Int {
        hands.enumerated().reduce(0) {
